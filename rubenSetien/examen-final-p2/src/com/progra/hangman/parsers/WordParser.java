...
public class WordParser implements Parser {
    ...
    public Word parse(String tokens) throws InvalidWordException {
            ...
            String tipe = parts[2].trim().toUpperCase();

            try {
                code = Integer.parseInt(parts[0]);
            } catch (NumberFormatException e) {
                throw new InvalidWordException("The code is not a valid number");
            }
            ...
            switch (tipe) {
                case "LARGA":
                    word = new LargeWord(code, words);
                    break;
                case "MEDIA":
                    word = new MediumWord(code, words);
                    break;
                case "CORTA":
                    word = new ShortWord(code, words);
                    break;
                default:
                    throw new InvalidWordException("The tipe of word is not valid");
            }
