...
public class WordParser implements Parser {
    ...
    public Word parse(String tokens) throws InvalidWordException {
        ...
        String splitTokens[] = tokens.split(regex);

        int wordId = splitTokens[0];
        String word = splitTokens[1];
        String wordLevel = splitTokens[2]; 

        if (splitTokens.length == 3) {
            if (wordLevel == "Larga") {
                LargeWord largeWord = new LargeWord(wordId, word);
            }
        } else (
            throw new InvalidWordException(e);
        )
